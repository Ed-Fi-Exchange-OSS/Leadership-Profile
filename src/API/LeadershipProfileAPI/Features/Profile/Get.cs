    ...
    public static class Get
    {
        ...
        public class QueryHandler : IRequestHandler<Query, Response>
        {
            ...
            public async Task<Response> Handle(Query request, CancellationToken cancellationToken)
            {
                ...
                response.Education = await _dbContext.StaffEducations.Where(o => o.StaffUniqueId == request.Id)
                    .ProjectTo<TeacherEducation>(_mapper.ConfigurationProvider).ToArrayAsync(cancellationToken);

                response.ProfessionalDevelopment = await _dbContext.StaffProfessionalDevelopments.Where(o => o.StaffUniqueId == request.Id)
                ...
                var criteria = await _dbContext.ProfileCategory
                    .ProjectTo<Category>(_mapper.ConfigurationProvider).ToArrayAsync(cancellationToken);

                var subcriteria = await _dbContext.ProfileSubCategory
                    .ProjectTo<SubCategory>(_mapper.ConfigurationProvider).ToArrayAsync(cancellationToken);

                var scores = await _dbContext.ProfileScoresByPeriod
                    .ProjectTo<ScoresByPeriod>(_mapper.ConfigurationProvider).ToArrayAsync(cancellationToken);
