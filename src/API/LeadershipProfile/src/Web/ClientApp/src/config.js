...
function config() {
    ...
    //  const API_URL = process.env.NODE_ENV === 'production' ? new URL(`${getUrl.protocol}//${getUrl.host}/api/`) 
    //     : new URL('https://garlandleadership.developers.net/api/');
    // const API_URL = process.env.NODE_ENV === 'production' ? new URL(`${getUrl.protocol}//${getUrl.host}/api/`) : new URL('https://localhost:44447/api/');
    const API_URL = new URL(`${getUrl.protocol}//${getUrl.host}/api/`);
