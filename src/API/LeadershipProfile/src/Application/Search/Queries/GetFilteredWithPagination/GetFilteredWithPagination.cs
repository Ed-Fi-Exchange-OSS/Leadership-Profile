...
public class GetFilteredWithPaginationQueryHandler : IRequestHandler<GetFilteredWithPaginationQuery, Response<SearchResultDto>>
{
    ...
    private static string ClauseAspires(ProfileSearchRequestAspires aspires)
    {
        if (aspires != null && aspires.Values?.Count > 0)
        {
            var whereAspires = aspires.Values.Any() ? $"InterestedInNextRole in ({string.Join(",", aspires.Values)})" : string.Empty;
